...
Since GMT only considers each feature once and processes it using relevant module (e.g., 'psbasemap', 'grdcontour', 'psscale'), its running speed is high. Such technical advantage enables processing the grids that cover large geographic areas in a few seconds. With prior knowledge of the geological and geophysical structure of the seafloor, GMT is fast and achieves good mapping performance. since the algorithm adjusts the projection parameters and sets focus on the main seafloor features of the study area in the required scale and projection. Furthermore, the GMT-based framework performs well for mapping the location, extent and borders of the mid-ocean ridges and major basins, because the redundant bathymetric features can be removed by cartographic generalisation, e.g., by selecting the gap in the isolines. This enables to find strongly relevant and nonredundant features, highlight the correlation between the geophysical setting, gravity and magnetic anomalies, and compare them with geodetic anomalies of the seafloor. Hence, seafloor structures can be mapped and visualised on the map layouts using effective GMT approach. 

\subsubsection{Mapping magnetic anomalies}

In mapping of the magnetic anomalies, the two different grids were used -- the WDMAM and the EMAG2. The first was subset from the global grid embedded in the GMT using the following code: $gmt grdcut @earth_wdmam_03m -R50/100/-70/-30 -Gker_mag.nc$, and the second was obtained from the existing raster file of EMAG 2-minute resolution grid as follows: $"gmt grdcut EMAG2_V2.grd -R50/100/-70/-30 -Gker_mag.nc"$. This illustrates that data processing in GMT is flexible and possible using both ways: by handling existing files as stored data, and using embedded grids which are available in the GMT system using remote access to these files. Afterwards, the images were visualised using the 'grdimage' module with color palettes adjusted according to the range of data distribution over the Kerguelen Plateau and adjacent south-west region of the Indian Ocean using the following code: 'gmt makecpt -Cmag.cpt -V -T-500/500 > myocean.cpt'. The remaining parts of the GMT scripts are presented for both maps visualised using the WDMAM and the EMAG2, respectively, in Listing \ref{lst09} and \ref{lst10} in the Appendix.
