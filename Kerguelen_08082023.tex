...
In particular, the present study aims at investigating the geophysical and topographic setting of the Kerguelen Plateau using the integrated datasets that cannot be achieved with the separate use of data. Ten new maps are presented and described to visualise spatial variations, reveal correlation and capture the matches between the geophysical and topographic setting of the Kerguelen region to continue and contribute to the existing cartographic works on this area \cite{Nougier,Petit,RECQ1989133}. Given the remote location of the Kerguelen Archipelago, one of the most isolated places on Earth, and associated difficulties and high cost of geological and geophysical sampling, the integrated use of the satellite derived data enables to get better insights into the structure of the Kerguelen Plateau within the south-west Indian Ocean.

%----------- section ----------->
\section{Study area}

The Kerguelen Plateau presents a large topographic elevation which extends on $6500 km^2$ \cite{Operto}. The Kerguelen Archipelago formed on the plateau forms a shallow submarine plateau. On the northwest, the archipelago is limited by the Crozet Archipelago which is located 1250 km \cite{BRETON2013126}, to the north -- by the African-Antarctic Basins, to the northeast -- by the Australian-Antarctic Basin, and to the south -- by the Antarctic. Further adjacent lands include the Enderby Land, Kemp Land, Mac Robertson Land, Princess Elizabeth Land, Wilhelm II Land and Queen Mary Land within the Antarctic, Figure \ref{fig02}. 
...
The Kerguelen Plateau belongs to the French Southern and Antarctic Lands with protected environment \cite{Rue1931,Faivre,Dupouy}. High environmental value of the south-west Indian Ocean is explained by the unique wildlife structure in this region which includes vulnerable species. The remote location of the archipelago -- 4.000 km from South Africa and Australia  \cite{Rue1932b,fishes8040174} created conditions to preserve the unique flora and fauna of this region. Rare plant species and high biodiversity of the Kerguelen Islands is associated with soil developed on the basaltic basement and specific geologic substrate of volcanic origin, as well as the effects from Polar climate, and includes rare Antarctic species \cite{Rue1932a,Paulian,Aubert3723}. The distribution of fish communities and phytoplankton is affected by seasonal changes of the Antarctic circumpolar currents \cite{Park2009,Damerell,MONGIN2008880,Park,Park2014,POLLARD20071915,Pauthenet,Penna}. The steep bathymetric slopes and exposition of the Kerguelen plateau contributes to the strength and intensity of circulation through the increased speed of the ocean currents \cite{Rietbroek}. As a result, the combination of all these factors makes the ecosystems of the Kerguelen Archipelago unique and protected as an environmental heritage of the French Southern and Antarctic Lands \cite{Sombetzkib,Sombetzkia,Naim}. 

The origin of the Kerguelen Plateau is related to the hotspot associated with the the kinematics of Gondwana breakup in Cretaceous (ca. 120--110 Ma) and spreading between India and Antarctica in the south-western segment of the Indian Ocean \cite{Guglielmi}. Early surface expression of the Kerguelen Plume at the southern Kerguelen Plateau initiated in Mesozoic around 120 Ma \cite{Gaina}. Active submarine volcanism on the aseismic ridge and associated sedimentation processes since Cretaceous resulted in the formation of the basaltic basement of Kerguelen \cite{Henry}. The volcanism continued in the Kerguelen hotspot up to Tertiary (<40 Ma) and is now reflected in current geologic and mineralogical structure of the archipelago. Thus, nowadays, the archipelago is covered up to 85\% by basalts originated from the mantle plume of the hotspot and resulting from the crystallisation of the raised and cooled basaltic magma \cite{NICOLAYSEN2000313,Lacroix,Doucet}. 

Currently, it presents one of the World's largest volcanic plateaus with a total length of 2300 km. Occasional evidence of volcanism is still remaining on the Heard and McDonald Islands \cite{Li}. Morphologically, the Kerguelen Plateau is oriented in the NNW direction toward the Antarctic continental margin where it is constrained by the Princess Elizabeth Land. The structure of the Kerguelen Archipelago is asymmetric with notable variations in the two distinct parts: northern part (Heard, McDonald and Kerguelen Islands) is more shallow (<1000 m) compared to the southern segment (depths of 1500-2000 m) \cite{Ramsay}. 

%----------- section ----------->
\section{Materials and Methods}

All the maps have been plotted using scripting toolset Generic Mapping Tools version 6.1.1, \href{https://www.generic-mapping-tools.org/}{https://www.generic-mapping-tools.org/}. A key aspect of the GMT algorithms is that is considers each cartographic element by its parameters and adds new layers on the map regarding the target location which can be adjusted using refined flag options in special modules. Thus, a modular scripting approach of GMT differs it from GIS. 

%----------- subsection ----------->
\subsection{Data}

The materials used in this study as input cartographic grids include the following datasets. The marine geological data on age, spreading rates, and asymmetry of the ocean crust were derived from the NOAA high-resolution dataset \cite{Mueller}, Figure \ref{fig03}. The units of the dataset on spreading asymmetry are expressed as \% of crustal accretion of the seafloor, i.e. symmetric spreading results in values of 50\% on conjugate ridge flanks. The bathymetric mapping was based on the ETOPO Global Relief Model \cite{NOAA}. The gravity grids were obtained from the EGM-2008 and EGM96 \cite{Pavlis}. The gravity data are based on the principle of gravity anomaly over various surfaces of the Earth which is reflected in the relationship between the topography, distribution of masses within the local relief and gravity values, and the development of gravitational potential in various locations of the Earth. 

The free-air and Bouguer gravity anomaly grids are derived from such data \cite{EGU,BRAITENBERG2021706}.
Specifically, the gravity field differs over the oceans due to variations in density. The highest anomalies arise from undulations on density, for instance at the over the heterogeneous seafloor or at the crust--mantle interface (Moho discontinuity) \cite{MCNUTT201920}. Such data enable to model the the gravity fields for analysis of the Earth structure. Compared to the older versions, current updates of gravity grid improve the altimetry-derived gravity anomalies. Although gravity grids do not indicate the topography directly, however, it provides essential information on relief over the Earth which gives an approximate in relationship between topographic and geophysical data and the agreement of surface gravity values with local geoid undulations \cite{CONDIE202281}. 

The data on sediment thickness are collected from the GlobSed dataset which shows the distribution of the sediment thickness in the World's Oceans \cite{Straume}. The magnetic data are derived from the two available information sources compiled originally from satellite and marine magnetic measurements: the Earth Magnetic Anomaly Grid (EMAG2) \cite{Maus} which has a resolution of 2 arc minutes, and the World Digital Magnetic Anomaly Map (WDMAM) which has a 3-minute resolution \cite{Lesur}. The gravity data were derived from the free-air global gravity grids \cite{Sandwell}.

The analysis of the geodetic data is useful in such complex terrains since it enables to identify major tectonic structures and topographic patterns. \hl{Besides the EGM grids, the }identification of \hl{the }lithospheric structures and tectonic blocks can \hl{also }be performed \hl{using} the satellite gravity data such as World Gravity Model (WGM) and Gravity Recovery and Climate Experiment (GRACE) \cite{EMISHAW2023104775}. \hl{Furthermore, }the EGM and astrogeodetic vertical deflections are useful for modelling gravity and geopotential difference \cite{WANG2023}. \hl{Finally, }processing \hl{the }terrestrial and satellite geodetic data can be employed for solving theoretical issues of geodesy such as altimetry–gravimetry boundary-value problem \cite{Lehmann}. Other examples \hl{of implications of the geodetic datasets }include \hl{the} use of the satellite-derived data from Cryosat-2 and altimetry with ship-measured gravity data for estimating marine gravity anomalies \cite{NGUYEN2020505}.
...
Mapping age, age uncertainty, spreading rates and spreading asymmetries of \hl{the }Kerguelen Plateau and the \hl{southwest} Indian Ocean basins is performed based on the raster grids with 2 minute resolution, Figure \ref{fig04}.  
...
The grids are \hl{reprojected} to Lambert Azimuthal Equal-Area projection using data from the major features in the basin of Indian Ocean. \hl{The vector lines of the }mid-ocean ridges\hl{, tectonic plates and lithospheric} plates are added using \hl{the }'psxy' module to show the borders of \hl{the }Indian, African and Australian plates: 'gmt psxy -R -J TPIndian.txt -L -Wthickest,red -O -K >> \$ps', Figure \ref{fig04}. Spreading half rates of the ocean crust are visualised based on the 2 arc min netCDF grid v.3.6 (NOAA). \hl{Likewise, the }tectonic slab contours are added using the 'psxy' module \hl{of the GMT using} the following command: 'gmt psxy -R -J file.gmt -Wthick,darkmagenta -O -K >> \$ps'. \hl{In such a way, }several grids are combined and represented through plotted raster grids to show the age-depth \hl{relationships} in the seafloor \hl{around the Kerguelen and southwest Indian Ocean}. The \hl{full} scripts are presented in \hl{the }Appendix \ref{App} \hl{of this study with }Listing \ref{lst02} \hl{used }for mapping age of the oceanic lithosphere over Kerguelen Plateau, Listing \ref{lst03} -- for mapping asymmetries in crustal accretion on conjugate ridge flanks over Kerguelen Plateau, Listing \ref{lst04} -- for visualisation of spreading half rates of the oceanic lithosphere in Kerguelen Plateau and SW Indian Ocean and Listing \ref{lst05} -- for mapping crustal age uncertainty of the oceanic lithosphere over the Kerguelen Plateau, Figure \ref{fig05}.

\subsubsection{Mapping sediment thickness}

A representation of the GlobSed 5-arc-minute total sediment thickness grid is extracted from the global raster grid \hl{on} a target location of \hl{the }Kerguelen \hl{Archipelago }area \hl{and surrounding regions }using 'gmt grdcut GlobSed-v2.nc' command which selects the file. The amplitude of values is checked using \hl{the Geospatial Data Abstraction Library (}GDAL\hl{)} library embedded in GMT as follows: 'gdalinfo file.nc -stats'. Given this extent of the values, the colour palette table is defined accordingly and presented using \hl{the }'psscale' module with defined \hl{the }parameters of \hl{the geographic }location \hl{for each} element on the map as follows: '-Dg-33/-59+w15.4c/0.4c'\hl{. For instance, here the} '-Dg-33/-59' \hl{command} defines the position of the colour scale on the map by coordinates. The grid annotations and graticules are added \hl{using the} 'psbasemap' \hl{GMT module. The} rest of the script with added comments for brief explanations is presented in Listing \ref{lst06} \hl{in the Appendix of this study. This listing was used for plotting the map in Figure} \ref{fig06}. 

\subsubsection{Geophysical mapping}

To highlight \hl{the }features and subsurface characteristics not visible from \hl{the geological }data, \hl{the }maps of gravity anomalies were prepared using gravity datasets \hl{in the Lambert Equal-Area Azimuthal Projection for conformity with other maps}. The full scripts for plotting geophysical maps are presented in Appendix in Listing \ref{lst07} for free-air gravity anomalies and Listing \ref{lst08} for vertical gravity gradient. The most essential lines of the code are as follows. For free-air gravity, the module 'img2grd' was first applied to convert the image from \hl{the} .IMG into GRD format. The projection was defined using '-JU43/6.0i' flag in 'grdimage' module and the region extent is clipped by "-R40/110/-70/-20" which passed to the next modules accordingly using '-R -J' flags. The vertical gravity gradient was plotted using the satellite-derived grid originally obtained from \hl{the }altimeters CryoSat-2, Jason-1/2 \cite{Harper} which enables to identify ridge propagation on the seafloor.

Since GMT only considers each feature once and processes it using relevant module (e.g., 'psbasemap', 'grdcontour', 'psscale'\hl{)}, its running speed is \hl{high. Such technical advantage} enables processing the grids \hl{that cover} large geographic areas in a few seconds. With prior knowledge of \hl{the }geological and geophysical structure of the seafloor, GMT is fast and achieves good \hl{mapping }performance. since the\hl{ }algorithm\hl{ adjusts the} projection parameters and \hl{sets} focus on the main seafloor features of the study area in \hl{the required} scale and projection. \hl{Furthermore, the} GMT-based framework\hl{ }performs well \hl{for mapping} the location, extent and borders of the mid-ocean ridges and major basins, \hl{because the }redundant \hl{bathymetric} features can be removed by \hl{cartographic }generalisation\hl{, e.g., by selecting the gap in the isolines}. This enables to find strongly relevant and nonredundant features\hl{, }highlight \hl{the }correlation between \hl{the }geophysical setting, gravity and magnetic anomalies\hl{, and compare them with }geodetic anomalies\hl{ }of the seafloor. \hl{Hence, seafloor structures} can be mapped and visualised on \hl{the} map layouts \hl{using effective GMT approach}. 

\subsubsection{Mapping magnetic anomalies}

In mapping of the magnetic anomalies, \hl{the }two different grids \hl{were used -- the} WDMAM and the EMAG2. The first was subset from the global grid \hl{embedded in the GMT }using \hl{the following }code: $gmt grdcut @earth_wdmam_03m -R50/100/-70/-30 -Gker_mag.nc$\hl{, }and the second was obtained from the \hl{existing raster file of }EMAG 2-minute resolution grid as follows: $"gmt grdcut EMAG2_V2.grd -R50/100/-70/-30 -Gker_mag.nc"$. \hl{This illustrates that data processing in GMT is flexible and possible using both ways: by handling existing files as stored data, and using embedded grids which are available in the GMT system using remote access to these files. Afterwards, }the images were visualised using \hl{the }'grdimage' module with color palettes adjusted \hl{according} to the range of data distribution over the \hl{Kerguelen} Plateau and \hl{adjacent south-west region of the }Indian Ocean using \hl{the following }code: 'gmt makecpt -Cmag.cpt -V -T-500/500 > myocean.cpt'. The remaining parts of the GMT scripts are presented for both maps visualised using \hl{the }WDMAM and the EMAG2\hl{, respectively,} in Listing \ref{lst09} and \ref{lst10} \hl{in the Appendix}.

\subsubsection{Mapping geoid anomalies}

The \hl{map of the} geoid \hl{inundations is} completed \hl{using} the EGM96 \hl{dataset} using \hl{the }lines of code in a GMT script where each module defines certain features on a map. To make mapping process effective, the scripts are run from the console for \hl{the} visualization of the geoid and adjusted accordingly to the target cartographic elements: projections, grids, colour palettes, raster extent and annotations, etc. To this end,\hl{ }these cartographic elements \hl{were also defined }stepwise on\hl{ }the presented maps of\hl{ }geoid, as demonstrated in \hl{a }script placed in the Appendix \ref{App} of this study in Listing \ref{lst11}\hl{. }Such strategy of data visualization \hl{used in} GMT ensures high automation of data processing which outperforms \hl{the }conventional mapping tools.

\hl{Strongly} relevant \hl{features, such as geoid undulation,} seafloor isochrons, basement depth isolines, and asymmetries in crustal accretion\hl{ }are associated \hl{with the} asthenospheric flow from mantle plumes of Kerguelen to spreading \hl{Southwest} and \hl{Southeast} Ridges. \hl{Mapping such data enables to reveal} information about the \hl{seafloor}attributes \hl{and compare it with the geologic} structure and \hl{geodynamics. Therefore, }mapping \hl{multi-source topographic and geophysical datasets enables to compare such} features using comparable projection\hl{. }In reality, however, seafloor features are rarely identical due to many factors affecting the bathymetry but may be strongly correlated. \hl{To this end}, the \hl{GMT }scripts can be adjusted to other scale and map the features in the enlarged view for detailed order of visualisation. To evaluate the impact of geophysical and geological setting on seafloor structure using the performance of GMT scripts,\hl{ }a number of cartographic trials \hl{ and re-projections was generated }in which each map \hl{represented the }selected seafloor features with various level of details, colour table for tuned visualisation. \hl{The optimal projection was selected as Lambert azimuthal equal-area, which was applied for all the maps for comparability and conformity.}

%----------- section ----------->
\section{Results and Discussion}

\hl{Automatic matching of topographic and geophysical grids with high accuracy is essential for complex geologic-tectonic investigations. This paper demonstrated the use of }scripting algorithms of GMT \hl{which }were applied for plotting 10 thematic maps \hl{covering the south-west segment of Indian Ocean and East Antarctic} using bathymetric, geodynamic and high-resolution geophysical datasets on gravity (Figure \ref{fig07} and \ref{fig08}). 

These maps reveal the structure of the seafloor in the Kerguelen Plateau and the surroundings region\hl{. }The results confirm that the dynamics of \hl{the }seafloor development reflected in \hl{the maps of }oceanic crust age, asymmetries, spreading rates of the south-west and southeast Indian ridges are related to major geophysical setting as reflected on the topographic, magnetic and gravity grids. Furthermore, the volcanic activity of the Kerguelen hotspot \hl{makes the} impact \hl{on }the distribution of \hl{the }magnetic anomalies, which supports relevant previous studies \cite{Fanjat,Weissel,Jokat}. Overall, the results demonstrate that \hl{the }GMT scripting is a powerful and stable cartographic method \hl{that performs efficiently }for geophysical and bathymetric seafloor mapping. In the sections below,\hl{ }the \hl{obtained }results \hl{are discussed }on relevant maps with provided comments on the essential features and traits of the seafloor around the Kerguelen Plateau, south-west Indian Ocean and \hl{the }East Antarctic. 

%----------- subsection ----------->
\subsection{Ocean floor formation}

\hl{The} general physical-geographic structure of the Kerguelen Plateau is visible in \hl{the} map of Figure \ref{fig01}\hl{. It shows a morphological} orientation of the archipelago in \hl{the }NW-SE\hl{ }direction and a total extent surpassing 2000 km in length. Northern and southern parts of the plateau are asymmetric where less expressed southern part is older and lie in deeper water in the topographically \hl{downlifted} areas. Age, spreading rates and spreading symmetry of the ocean crust indicate the evolution steps \hl{of the} ocean floor formation in \hl{southwest} Indian Ocean and \hl{around }the Kerguelen Plateau. The geodynamics setting of the oceanic crust (Figures \ref{fig02}, \ref{fig03}, \ref{fig04} and \ref{fig05}) shows \hl{a} strong relationship \hl{between} the Kerguelen Plateau \hl{and} the \hl{two mid-ocean ridges.} 
...
The age of the ocean crust was determined by \hl{the }interpolation of \hl{the }adjacent seafloor isochrons oriented towards the direction of seafloor spreading, Figure \ref{fig02}. \hl{Such correspondence highlights that }unique geophysical setting of the Kerguelen Plateau underlain by \hl{the }oceanic crust is strongly associated with its tectonic origin \hl{associated with volcanic hotspot }and \hl{geologic }history. The \hl{formation of the Southwest and the Southeast }Indian Ridges \hl{is related to the} uplift of the Kerguelen Plateau as a remnant of the Mesozoic oceanic basin existing after the separation of Gondwana. The analysis of the bathymetric map with geodynamic maps shows that seafloor heterogeneity \hl{around the Kerguelen Archipelago }correlates with \hl{the }seafloor spreading rates where rougher basement is formed in the areas with \hl{the }low half-spreading rate threshold (30-35 mm/year and lower) \hl{which can be revealed by comparison of maps in} Figure \ref{fig01} and Figure \ref{fig04}. Thus, the heterogeneity in seafloor patterns varies significantly in various basins of the Indian \hl{Ocean} depending on the geodynamic setting and \hl{the geologic }development of \hl{the }ocean crust.

Gridded map of age of the ocean crust around the Kerguelen Plateau (Figure \ref{fig02}) shows a correlation with the observed spreading half rates of the lithosphere (Figure \ref{fig04}). Originally formed as a single structure\hl{, }Kerguelen was then split by \hl{the }seafloor spreading in the south-west sector of the Indian Ocean \hl{which resulted in the formation of the two segments of the archipelago} \cite{BRADLEY1988243}. Moreover, \hl{the difference in the }volcanic activity on the northern and central parts of the Kerguelen Plateau underlying the Heard Island proves that it is located on a hotspot \hl{with various parts of the islands experiencing the effects from the mantle plume in different scale} \cite{Duncan2016,Plenier,HOUTZ197795}. Such unstable position impacts the geodynamic patterns over the Kerguelen \hl{Islands}, with higher uncertainty of age of the oceanic crust, Figure \ref{fig05} compared to the adjacent areas. Further, the structure of the oceanic crust beneath the Kerguelen Plateau is similar to the crust beneath the aseismic ridges such as the Crozet Rise and the Madagascar Ridge, which proves that it originates by active volcanism from a hot spot, as noted earlier \cite{Recq}.

%----------- subsection ----------->
\subsection{Sediment thickness}

Mapping sediment thickness using \hl{the }5-arc-minute GlobSed grid relies on the approximated modelling \hl{which highlights} the sedimentation trends around \hl{the }Kerguelen \hl{Archipelago}, Figure \ref{fig06}. The distribution of \hl{the }sediment thickness correlates with the age of the underlying oceanic lithosphere and its latitude\hl{, which can be noted} by \hl{the }comparison of maps in Figure \ref{fig02} and Figure \ref{fig06}\hl{. Such correspondence is }especially visible \hl{for} higher values of \hl{the }sediment thickness near the shorelines of Antarctic, Amery Ice Shelf (3000-4000 m), Enderby Land (over 4000 m) and \hl{the }Kerguelen Plateau (2000-3000 m). Higher level of sediment thickness in these areas may also indicate earlier processes of the subaerial erosion which took place before the subsidence and associated sedimentation. 

Rifting during the Late Paleogene resulted in changes in \hl{the }Tertiary sediment facies of Kerguelen which are affected by the evolution of the Antarctic environment \cite{Froehlich}. Sediments covering the Kerguelen Plateau include pillow lavas, tuffaceous sediments and marine siltstones which were deposited since late Miocene \cite{Duncan}. They are continued as \hl{a} thick sequence of the Cenozoic sediments (over 5000 m\hl{)} within the Enderby Basin to the south-west off the Kerguelen Plateau, Figure \ref{fig06}. \hl{The correlations reported between the sediment thickness (Figure} \ref{fig06}) \hl{and the age of the oceanic lithosphere (Figure} \ref{fig02}) \hl{demonstrate the role of the ocean floor formation on the accumulation of the sediments.} 

\hl{Other important factors increasing }the \hl{accumulation of the sediments} around \hl{the }Kerguelen and \hl{the }adjacent area \hl{include }glacial processes and the turbidity of the ocean currents\hl{. Hence, intense} circulation results in \hl{the accumulation of the }large sediment fields with values over 3000 m. This results from the \hl{developed} Antarctic circumpolar currents which started \hl{initially }around the Eocene--Oligocene \hl{and continued until present time as the strongest current system in the oceans directed clockwise around the South Pole affecting the adjacent sub-Antarctic regions such as Kerguelen }\cite{Huang,ROBERT200237}. A general orientation of maximal sedimentation areas is coherent to the sediment-filled troughs stretching in NW-SE-direction which are associated with general NW-SE orientation of Kerguelen and axes of the tectonic faults. High values in sediment thickness around the Kerguelen Plateau controured by the ridge isolines its eastern margin is associated with the depositions from the bottom currents directed westwards.
...
\hl{The visualised }marine gravity field over the Kerguelen Plateau region and \hl{the }adjacent areas of the south-west Indian Ocean are shown in Figure \ref{fig07}. The comparison of \hl{the }gravity roughness with \hl{the }map of the half-spreading rates (Figure \ref{fig04}) and sediment thickness (Figure \ref{fig06}) shows the relationship between the speed of the spreading of mid-ocean ridges and roughness of \hl{the }seafloor basement which is explained by the process of mid-ocean ridge formation. Other factors \hl{include} the associated magma flows, spreading directions in Mesozoic and isochron orientations of \hl{the }age of the oceanic crust which affect current bathymetric and gravity patterns \cite{Whittaker}. Furthermore, the gravity highs \hl{around} the Kerguelen Plateau and \hl{the }Heard Island correspond to the maximal bathymetric elevations and indicate \hl{on }the seamounts formed of Miocene basalts erupted during \hl{the }volcanic activity in the southern Indian Ocean and resulted in a large igneous province \cite{Weis}.

The analysis of maps shows that \hl{the values of the }free-air gravity over the Kerguelen are higher that in the \hl{surrounding areas }and reach up to 80 mGal (dark orange colour, Figure \ref{fig08}). \hl{In contrast, }lower values are associated with \hl{the }bathymetric depressions \hl{and have values of }-40 to -60 mGal. \hl{Furthermore, }the abyssal plain is characterised by \hl{the }medium values of 0-20 mGal\hl{, }Figure \ref{fig08}. This well illustrates the correlation of \hl{the free-air }gravity anomalies with the distribution of topographic highs and depressions \hl{on the seafloor }since they are strongly influenced by a gravitational effect of the \hl{distributed }topographic masses \hl{since they are caused by differences in elevation}. 
...
\hl{Figure} \ref{fig08} \hl{shows the outputs of the mapping for vertical gradient over the Kerguelen and the southwest Indian Ocean where the effects of different locations are presented.} \hl{The visualised map demonstrate that the crucial property of gravitational systems like free-air gravity is not only the inclusion of geographic location and the latitude of the selected measured regions, but also the altitude of the Earth's surface since greater altitude implies greater distance from the Earth's centre with affects gravity values. }Vertical gravity gradient \hl{identifies the }variations in gravity with the change in topographic elevations are shown in Figure \ref{fig08}\hl{. The comparison of gravity datasets gives additional information on the distribution of major geological and seafloor structures with regard to the variations of geophysical fields}. 

\hl{Moreover, the comparison of the vertical gradient and free-air gravity  map (Figure} \ref{fig07} \hl{and Figure} \ref{fig08}) \hl{with the topographic map (Figure} \ref{fig01}) \hl{illustrates the effects of seafloor structures and their distribution of the oceanic bed on gravity and showed that the highest values correspond to the Kerguelen Plateau and other rises, while lower values are generally associated with depressions.}
...
The EMAG-2 and WDMAM grids used for plotting the magnetic anomalies show various levels of details of the grids which enable to reveal the subsurface structure of the seafloor around the Kerguelen Plateau and \hl{the }composition of the Earth's crust in \hl{the southwest} Indian Ocean. Here, \hl{the }magnetic fabric data correlate with \hl{the }hotspot activities and active volcanism which is notable over the central and northern parts of the archipelago \cite{HENRY2003153,Xu,Pettersen}. Moreover, values of \hl{the }intermediate crustal thickness in the oceanic crust beneath the Kerguelen Plateau and large-amplitude magnetic anomalies over the archipelago evidence that the plateau is oceanic in origin and is associated with the plate volcanism caused by \hl{the }tectonic plate activity, which is also \hl{reported} in \hl{earlier} studies \cite{Direen,Charvis1995}. Therefore, the distinct magnetic patterns visible in maps in Figure \ref{fig10} corresponds to \hl{the }high hotspot activity and associated\hl{ }lava flows. 
...
\hl{The effects from deeper mass, asthenospheric upwelling and convection from mantle plumes as geodynamic processes and affect magnetic properties of the surface. Moreover, the analysis of data enables to better understand the impact of geophysical setting on  on the distribution of positive (over the Kerguelen Islands) and negative undulation (eastern regions of the study area and south-west Australia), which enables to evaluate the variations of geophysical grids using the comparative analysis of maps. In this way, cartographic representation of the geophysical and magnetic datasets provides advanced methods to obtain data on the seafloor formation and interrelated geophysical process.}

%----------- subsection ----------->
\subsection{Geoid models}
The geopotential \hl{model} over \hl{the }Kerguelen based on the EGM96 dataset \hl{is} shown in Figure \ref{fig11}. The variations of geoid over the Kerguelen Plateau point at \hl{the }existing isostatic compensation of the archipelago due to \hl{the }low-density mantle. Thus, high anomalies in \hl{the }geoid level above the Kerguelen Plateau are explained by \hl{a} high volcanism associated to \hl{the} ridge formation on \hl{the }hot lithosphere, which is reflected in the extraordinary thickness below \hl{the }Kerguelen with geoid values of above 40 m (red areas in Figure \ref{fig11}) which exceeds normal thickness of the oceanic crust. This proves previous investigations on geoid in the Kerguelen Archipelago that reported anomalous thickness in this area \cite{RECQ1987301}. Such isostatic compensation \hl{is }associated with the anomalously high geoid values \hl{and }is related to the topographically rough elevated surfaces \hl{in} regions of active tectonic uplift \hl{which identifies processes in }upper mantle \cite{Coblentz}. 
