...
The origin of the Kerguelen Plateau is related to the hotspot associated with the the kinematics of Gondwana breakup in Cretaceous (ca. 120-110 Ma) and spreading between India and Antarctica in south-western segment of the current Indian Ocean \cite{Guglielmi}. Early surface expression of the Kerguelen Plume at the southern Kerguelen Plateau initiated in Mesozoic around 120 Ma \cite{Gaina}.
 
Active submarine volcanism on the aseismic ridge and sedimentation processes since Cretaceous resulted in the formation of the basaltic basement of Kerguelen  \cite{Henry}. The volcanism continued in the Kerguelen hotspot up to Tertiary  (<40 Ma) and is now reflected in current geologic and mineralogical structure of the archipelago which is covered up to 85\%  by basalts originated from the mantle plume of the hotspot and resulting from the crystallisation of the raised and cooled basaltic magma \cite{NICOLAYSEN2000313,Lacroix,Doucet}. Nowadays, Kerguelen Archipelago presents a shallow submarine plateau limited to the northwest by the Crozet Archipelago which is located WNW 1250 km \cite{BRETON2013126}, to the north -- by the African-Antarctic Basins, to the northeast -- by the Australian-Antarctic Basin, and to the south -- by the Antarctic. The adjacent lands here include the Enderby Land, Kemp Land, Mac Robertson Land, Princess Elizabeth Land, Wilhelm II Land and Queen Mary Land, Figure \ref{fig02}. 
...
Currently, it presents one of the world's largest the volcanic plateaus (the total length of 2300 km) with occasional evidence of volcanism remaining on the Heard and McDonald Islands. \cite{Li}. Morphologically, Kerguelen Plateau is oriented in the NNW direction toward Antarctic continental margin where it is constrained by the Princess Elizabeth Land. Kerguelen Archipelago is asymmetric in structure, with notable variations in two parts. Northern part (Heard, McDonald and Kerguelen Islands) is more shallow (<1000 m) compared to the southern segment (depths of 1500-2000 m) \cite{Ramsay}. 
...
Vertical gravity gradient anomalies over Kerguelen Plateau region showing variations in gravity with the change in topographic elevations are visualised in Figure \ref{fig08}. Gravity data is useful to reveal tectonic structures and topographic patterns. For example, mapping lithospheric structures such as tectonic blocks can be performed based on the satellite gravity data including World Gravity Model (WGM) and Gravity Recovery and Climate Experiment (GRACE) \cite{EMISHAW2023104775}. The Earth Gravity Model (EGM) and astrogeodetic vertical deflections were employed for modelling gravity  and geopotential difference \cite{WANG2023}. A joint processing of terrestrial and satellite geodetic data was applied for solving theoretical issues of geodesy such as altimetry–gravimetry boundary-value problem \cite{Lehmann}. More examples include integrated use of the satellite-derived data from Cryosat-2 and satellite altimetry with ship-measured gravity data for estimating marine gravity anomalies \cite{NGUYEN2020505}. 

%----------- subsection ----------->
\subsection{Magnetic anomalies over Kerguelen Plateau}

The anomaly of the magnetic intensity at an altitude of 5 km above mean sea level over Kerguelen is shown in Figure \ref{fig09}. The crustal volume contributes to the decreased amplitude of the magnetic anomaly around Kerguelen as can be seen as dark blue areas in Figure \ref{fig09} with lower values of around -500 mGal. 
