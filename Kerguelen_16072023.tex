...
All the maps have been plotted using scripting toolset Generic Mapping Tools version 6.1.1, \href{https://www.generic-mapping-tools.org/}{https://www.generic-mapping-tools.org/}. A key aspect of the GMT algorithms is to consider that each cartographic element evaluated by its parameters adds new layers on the map regarding the target location which can be adjusted using refined flag options in special modules. Thus, a modular scripting approach of GMT differs it from the traditional GIS. The final map is completed from the aggregation of lines of code in a script where each module defines certain features on a map. To make mapping process effective, a script is run from the console for each visualization and adjusted accordingly to the target cartographic elements: projections, grids, colour palettes, raster extent and annotations, etc. To this end, we also define these cartographic elements stepwise on each of the presented maps, as demonstrated in script placed in the Appendix \ref{App} of this study. Such strategy of data visualization adopted by GMT ensures high automation of data processing which outperforms conventional mapping tools.
...
Spreading half rates of the ocean crust are visualised based on the 2 arc min netCDF grid v.3.6 (NOAA). The difference between the gridded age of the ocean crust around the Kerguelen Plateau (Figure \ref{fig02}) shows a correlation with the observed spreading half rates of the lithosphere (Figure \ref{fig04}).
