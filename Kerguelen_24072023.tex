...
\abstract{The volcanic Kerguelen Islands are formed on one of the world's largest submarine plateaus. Located in the remote segment of the southern Indian Ocean close to Antarctica, the Kerguelen Plateau is notable for complex tectonic origin and geologic formation related to the Cretaceous history of the continents. This is reflected in varying age of the oceanic crust adjacent to the plateau and highly heterogeneous bathymetry of the \textcolor{blue}{Kerguelen} Plateau \textcolor{blue}{with} seafloor structure \textcolor{blue}{differing for} the southern \textcolor{blue}{and} northern \textcolor{blue}{segments}. Remote sensing data derived from \textcolor{blue}{the} marine gravity and satellite radar altimetry surveys serve as an important source of information for mapping \textcolor{blue}{complex} seafloor features. \textcolor{blue}{This study incorporates} the geospatial information from NOAA, EMAG2 and WDMAM, ETOPO1 and EGM96 \textcolor{blue}{datasets} to refine the extent and distribution of \textcolor{blue}{the extracted seafloor} features. The cartographic joint analysis of topography, magnetic anomalies, tectonic and gravity \textcolor{blue}{grids} is based on \textcolor{blue}{the} integrated mapping \textcolor{blue}{performed} using Generic Mapping Tools (GMT) \textcolor{blue}{programming suite}. \textcolor{blue}{Mapping} of the submerged features (Broken Ridge, Crozet Islands, \textcolor{blue}{seafloor fabric}, orientation and frequency of magnetic anomalies) \textcolor{blue}{enabled} to analyse the \textcolor{blue}{their} correspondence with free-air gravity and magnetic anomalies, \textcolor{blue}{geodynamic setting} and seabed structure in south-west Indian Ocean. \textcolor{blue}{The} results show that integrating \textcolor{blue}{the} datasets using \textcolor{blue}{advanced} cartographic scripting language improves identification and visualization of the seabed objects. The \textcolor{blue}{results include} 11 new maps \textcolor{blue}{on the region covering the Kerguelen Plateau and southwest Indian Ocean. This study contributes} to increasing the knowledge of \textcolor{blue}{the seafloor} structure \textcolor{blue}{in} the French Southern and Antarctic Lands.
}
...
The direct surveys of seafloor observation are expensive to generate bathymetric data and require the use of complex costly equipment such as multi-beam echo-sounding systems for data sampling, generating and storage. At the same time, the reality calls for efficient datasets for \hl{visualisation} and analysis. Using high-resolution geophysical datasets provided by NOAA and USGS processed by GMT advanced scripting approach develops a cartographic processing pipeline for fast, automated and accurate mapping of seafloor. \hl{A balance between the topographic gradients and geophysical grids well illustrates the links between the seafloor patterns and the structure of the oceanic crust and the processes in the lithospheric mantle. From the cartographic point of view, this proves that the effective analysis of the geologic-geophysical datasets in the Earth sciences is not confined to using selected data (e.g., topographic maps) as a single parameter but also to properly select several datasets since the processes of deep mantle reflecetd in the geophysical data define the process of seafloor formation. }

\hl{The cartographic approach demonstrated in this study }makes possible \hl{data assimilation} and extend it not only on \hl{the }Indian Ocean but also towards other regions of the World Ocean. For instance, \hl{the }Pacific Ocean is notable for \hl{a }rich seafloor texture features such as vast abyssal plains, mid-ocean ridges, oceanic trenches, numerous seamounts and continental shelf which provide a \hl{reliable} source of information for matching potential correlation between \hl{the }geophysical and magnetic anomalies and bathymetric responses as heterogeneous seafloor pattern. \hl{With this regard}, the use of \hl{the }GMT\hl{ }for seafloor mapping in other regions of the World Ocean makes an ideal case for validating our cartographic scripting approach presented in the above methodological scripting framework. 
...
The above series of maps and \hl{the }comparison of \hl{the }geophysical setting over \hl{the }Kerguelen \hl{Plateau}performed using \hl{the }GMT conclude that scripts techniques outperform the GIS approach in terms of automation of \hl{the }cartographic workflow\hl{. Besides, }the compactness of the code using \hl{the }GMT syntax which can be reapplied with modified setting. When handling \hl{the }cartographic elements on maps and \hl{adjusting the }parameters of projection, the main drawback of \hl{the }GMT is that it needs to tune \hl{these }parameters\hl{ }in advance while writing the code\hl{. This is because} it cannot visualise the map prior to \hl{the }executed script \hl{being} a completely console-based program. \hl{Furthermore, the} GMT cannot discard the redundant features once plotted on a map\hl{. Therefore, }correcting \hl{the map }requires running the script anew. In GIS approach, for example, user's setting may change the appearance of map layout so that new colour palettes and details of \hl{the }bathymetric plotting with regard to \hl{the }isoline density and consistency may become redundant and be corrected on the fly. In contrast, as a\hl{ console-based} application, GMT requires modification of these parameters directly in the code lines of the script to handle \hl{cartographic issues}. 
...
\hl{GDAL} & \hl{Geospatial Data Abstraction Library} \\
